...
if ("mean_GC3s" %in% names(genomes)) {
  gc3_diag <- genomes %>%
    ...
    summarise(
      ...
      mean_GC3s = mean(mean_GC3s, na.rm = TRUE),
...
if (all(c("genome_gc", "mean_GC3s", "dikarya_status") %in% names(genomes))) {
  ...
  save_plot_pair(p_split_gc, "dikarya_non_dikarya_GC_GC3s_split_violin", args$output_dir, 9.5, 6.2, formats)
}
...
make_dikarya_panel <- function(metric, title, y_label, as_percent = FALSE) {
  ...
  ggplot(d, aes(status, value, fill = status)) +
    ...
    theme(
      axis.text.x = element_text(angle = 18, hjust = 1, size = 10.4),
      axis.text.y = element_text(size = 10.8),
      axis.title.y = element_text(size = 11.4),
      plot.title = element_text(size = 12.6, face = "bold", hjust = 0.5),
...
if ("mean_tAI" %in% names(genomes) && length(gc_predictors) > 0L) {
  ...
  if (nrow(tai_gc) >= 6L) {
    ...
    save_plot_pair(p_tai_gc, "dikarya_tai_vs_cds_gc_gc3", args$output_dir, 11.5, 6.2, formats)
    ...
    regression_summary <- tai_gc %>%
      ...
      group_modify(~ {
        ...
        if (is.null(fit)) return(tibble(n_genomes = nrow(.x), slope = NA_real_, r_squared = NA_real_, p_value = NA_real_))
        sm <- summary(fit)
        tibble(
          n_genomes = nrow(.x), slope = unname(coef(fit)[[2]]),
